...
app.post('/auth/register', (req, res) => {
    ...
    if (!username || !password || password.length < 6) {
        return res.status(400).json({ error: 'Username dan password (min 6 char) wajib diisi.' });
    }
...
app.get('/', (req, res) => {
    res.json({ 
        message: 'Selamat datang di Movie API. Akses /movies, /directors, atau /auth/register untuk memulai.',
        endpoints: ['/status', '/movies', '/directors', '/auth/register', '/auth/login', '/auth/register-admin']
    });
});
...
app.post('/movies', authenticateToken, (req, res) => { // tidak perlu admin
...
app.put('/movies/:id', [authenticateToken, authorizeRole('admin')], (req, res) => { // tambahan admin disini
    const { title, director, year } = req.body;
    if (!title || !director || !year) {
        return res.status(400).json({ error: 'title, director, year wajib diisi untuk update' });
    }
...
app.put('/directors/:id', [authenticateToken, authorizeRole('admin')], (req, res) => {
    const { name, birthYear } = req.body;
    if (!name || !birthYear) {
        return res.status(400).json({ error: 'name dan birthYear wajib diisi untuk update' });
    }
